...
const settings = ["Profile", "Category", "Dashboard", "Logout"];
...
function Navbar() {
  ...
  const handleLogout = async () => {
    const response = await ApiCallGateway.logout();
    console.log("logout res ", response);
    if (response?.status !== 200) {
      failureAlert("Logout Failed", response?.data);
    } else {
      successAlert("Success", "Logout Successfully");
  ...
  return (
    <AppBar position="sticky" sx={{ backgroundColor: "#2463EB" }}>
